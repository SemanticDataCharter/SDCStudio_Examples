"""
Wizard Views for statepopulation.
...
class WizardStartView(View):
    ...
    def get(self, request):
        ...
        return redirect('statepopulation:wizard-step', step=first_step)
...
class WizardStepView(View):
    ...
    def get_template_name(self, step: int) -> str:
        ...
        return f'statepopulation/wizard/step_{step}.html'
    ...
    def post(self, request, step: int):
        ...
        if 'prev' in request.POST:
            prev_step = WizardStepConfig.get_prev_step(step)
            if prev_step >= 0:
                return redirect('statepopulation:wizard-step', step=prev_step)

        if 'save_draft' in request.POST:
            messages.info(request, "Draft saved. You can continue later.")
            return redirect('statepopulation:instance-list')

        # Move to next step or complete
        next_step = WizardStepConfig.get_next_step(step)
        if next_step >= 0:
            return redirect('statepopulation:wizard-step', step=next_step)
        else:
            # This was the last step - complete the wizard
            return redirect('statepopulation:wizard-complete')
    ...
    def get(self, request):
        ...
        try:
            ...
            with transaction.atomic():
                instance = StatepopulationInstance(
            ...
            return redirect('statepopulation:instance-detail', pk=instance_id)
